...
import { UniversalBuffer } from '../GitBackendFs/utils/UniversalBuffer.ts'
...
export class InMemoryBackend implements GitBackend {
  ...
  async expandRef(ref: string, cache: Record<string, unknown> = {}): Promise<string> {
    const { NotFoundError } = await import('../../errors/NotFoundError.ts')
    const { parsePackedRefs } = await import('../../refs/packedRefs.ts')
  ...
  async getRemoteInfo(
    ...
    const { RemoteBackendRegistry } = await import('../../remote/RemoteBackendRegistry.ts')
    const { formatInfoRefs } = await import('../GitBackendFs/utils/formatInfoRefs.ts')
