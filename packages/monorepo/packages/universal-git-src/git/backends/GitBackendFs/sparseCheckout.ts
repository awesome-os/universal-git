...
export async function sparseCheckoutSet(
  ...
  if (treeOid) {
    // Update the worktree to reflect changes
    // We call checkout to update the working directory based on the new patterns
    // Pass force: true to overwrite/update existing files
    await this.checkout(worktreeBackend, treeOid, { force: true })
  }
}
