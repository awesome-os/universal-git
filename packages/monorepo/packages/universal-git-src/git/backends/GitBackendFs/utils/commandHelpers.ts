...
import { GitBackendFs } from '../GitBackendFs.ts'
...
export async function normalizeCommandArgs<T extends Record<string, unknown>>(
  ...
  */
  ...
  let worktree: GitWorktreeBackend | undefined = args.worktree
    ...
    // Try to get dir from worktree backend if available
    if (args.dir) {
      dir = normalize(args.dir)
    } else if (repo.worktreeBackend && 'getDirectory' in repo.worktreeBackend && typeof repo.worktreeBackend.getDirectory === 'function') {
      const worktreeDir = repo.worktreeBackend.getDirectory()
