...
export async function abortMerge({
  ...
  try {
    ...
    // Import detectObjectFormat once at the top
    ...
    const [headTree, index] = await Promise.all([
      ...
      (async () => {
        ...
        // Use detectObjectFormat with gitBackend (already imported at top)
        const objectFormat = await detectObjectFormat(repo.gitBackend)
    ...
    async function listWorkdirFiles(relativePath: string = ''): Promise<string[]> {
      const files: string[] = []
      try {
        if (!worktreeBackend) return files
    ...
    await pMap(operations, async (op) => {
      ...
      if (op.op === 'update') {
        ...
        try {
          log(`    Deleting file...`)
          if (worktreeBackend && 'remove' in worktreeBackend && typeof worktreeBackend.remove === 'function') {
            await worktreeBackend.remove(relativePath)
          } else {
            // Fallback: use write with empty content or delete via fs
            const fs = (gitBackend as any).getFs?.()
            if (fs && 'remove' in fs && typeof fs.remove === 'function') {
    ...
    let finalIndex: InstanceType<typeof GitIndex>
    // Use detectObjectFormat with gitBackend (already imported at top)
    const objectFormat = await detectObjectFormat(repo.gitBackend)
    if (indexBuffer.length === 0) {
    ...
    // Get fs from gitBackend if available
    const fs = (gitBackend as any).getFs?.() || (worktreeBackend as any)?.getFs?.()
    if (!fs) {
      log('Warning: fs not available, skipping merge state file cleanup')
    } else {
      await Promise.all([
