...
import { REFLOG_MESSAGES } from "../../../logs/messages.ts"
...
export async function _commit({
  ...
  if (repo?.gitBackend && 'getFs' in repo.gitBackend && typeof repo.gitBackend.getFs === 'function') {
    fs = repo.gitBackend.getFs()
  } else if (repo) {
    const repoAny = repo as any
    if (repoAny.__fs) {
      const { createFileSystem } = await import('../utils/createFileSystem.ts')
      fs = createFileSystem(repoAny.__fs)
    }
  }
  if (!fs && _fs) {
    const { createFileSystem } = await import('../utils/createFileSystem.ts')
  ...
  return indexLock.acquire(indexPath, async () => {
    ...
    if (!dryRun) {
      try {
        ...
        await runHook({
          gitBackend,
          fs: gitBackend ? undefined : (fs || undefined),
          gitdir: gitBackend ? undefined : (gitdir || undefined),
          hookName: 'pre-commit',
          context: {
            gitdir,
            workTree,
    ...
    if (!noUpdateBranch && !dryRun) {
      ...
      if (initialCommit && ref.startsWith('refs/heads/')) {
        ...
        if (repo) {
          ...
          const { writeRef } = await import('../../../refs/writeRef.ts')
        ...
        try {
          ...
          if (headRef && typeof headRef === 'string' && headRef.startsWith('ref: ') && headRef.includes(ref)) {
            ...
            if (repo) {
              ...
              // Pass oid as newOid so writeSymbolicRef doesn't need to resolve it again
              await repo.gitBackend.writeSymbolicRef('HEAD', `refs/heads/${branchName}`, oldOid, oid, repo.cache)
            } else {
              ...
              await writeSymbolicRef({ fs, gitdir, ref: 'HEAD', value: `refs/heads/${branchName}`, oldOid, newOid: oid, objectFormat })
            }
          }
        } catch {
