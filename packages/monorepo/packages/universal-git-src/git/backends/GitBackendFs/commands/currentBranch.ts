...
import { readSymbolicRef, resolveRef } from "../../../refs/readRef.ts"
...
export async function _currentBranch({
  ...
  // Get fs from backend if repo is provided, otherwise use provided _fs
  let fs: FileSystemProvider | undefined = _fs
  if (repo && !fs) {
