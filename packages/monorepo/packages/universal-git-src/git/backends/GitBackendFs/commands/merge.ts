...
export async function _merge({
  ...
  if (repo.gitBackend) {
    try {
      ...
      if (indexBuffer.length > 0) {
        // Get object format from config (or default to sha1)
        ...
        try {
          ...
          // Config doesn't exist or can't be read - default to sha1
          // This is okay - we can still check for unmerged paths
        }
        const index = await GitIndex.fromBuffer(indexBuffer, objectFormat)
        
        // Check for unmerged paths - this MUST happen before any other operations
  ...
  if (fastForward && baseOid === ourOid) {
    ...
    // Also skip index write if dryRun is true (don't modify repository state in dry run)
    if ((typeof mergeResult === 'string' || !abortOnConflict) && !dryRun) {
