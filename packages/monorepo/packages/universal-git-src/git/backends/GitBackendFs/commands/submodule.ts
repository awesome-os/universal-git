...
import type { HttpClient } from "../../../remote/types.ts"
...
export async function submodule({
  ...
  try {
    ...
    if (statusFlag) {
      ...
      const repoFs = repo.gitBackend.getFs()
      const headOid = await resolveRef({ fs: repoFs, gitdir: finalEffectiveGitdir, ref: 'HEAD' })
      const { object: commitObject } = await readObject({ fs: repoFs, cache: repo.cache, gitdir: finalEffectiveGitdir, oid: headOid })
      ...
      for (const submodule of submodules) {
        ...
        try {
          expectedOid = await resolveFilepath({ fs: repoFs, cache: effectiveCache, gitdir: finalEffectiveGitdir, oid: treeOid, filepath: submodule.path })
        } catch {
        ...
        if (submoduleDirExists && submoduleGitdirExists) {
          try {
            ...
            actualOid = await resolveRef({ fs: repoFs, gitdir: submoduleGitdir, ref: 'HEAD' })
    ...
    if (update) {
      ...
      try {
        submoduleCommitOid = await resolveFilepath({ fs: repoFs, cache: effectiveCache, gitdir: finalEffectiveGitdir, oid: treeOid, filepath: submodule.path })
      } catch (err) {
