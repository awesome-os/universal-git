...
export async function _checkout({
  ...
  gitBackend?: import('../../GitBackend.ts').GitBackend
  ...
  let gitBackend: import('../../GitBackend.ts').GitBackend
  let worktreeBackend: import('../../../worktree/GitWorktreeBackend.ts').GitWorktreeBackend | null = null
  let worktreeGitdir: string
  let worktreeDir: string
  let repo: import('../../../../core-utils/Repository.ts').Repository | null = null
  
  const resolveWorktreeDir = (backend: import('../../../worktree/GitWorktreeBackend.ts').GitWorktreeBackend | null, fallback?: string): string => {
    if (backend) {
  ...
  const resolveWorktreeGitdir = (
    backend: import('../../GitBackend.ts').GitBackend | undefined,
    fallback?: string
  ): string => {
  ...
  // CRITICAL: Read oldOid BEFORE any HEAD modifications to ensure accurate reflog
  let oldOid: string | undefined
  try {
    const headOid = await gitBackend.readRef('HEAD', 5, cache)
    // readRef returns null if HEAD doesn't exist, or the OID if it does
    // For reflog, we need a valid OID string, not null
      ...
      if (ref === 'HEAD') {
        ...
        try {
          ...
          if (oid) {
            ...
            // Pass oldOid and oid (newOid) for reflog
            await gitBackend.writeSymbolicRef('HEAD', `refs/heads/${defaultBranch}`, oldOid, oid, cache)
            originalErr = null // Clear error since we successfully resolved
          } else {
          ...
          try {
            ...
            if (branches.length > 0) {
              ...
              if (oid) {
                ...
                // Pass oldOid and oid (newOid) for reflog
                await gitBackend.writeSymbolicRef('HEAD', `refs/heads/${firstBranch}`, oldOid, oid, cache)
                originalErr = null // Clear error since we successfully resolved
              } else {
