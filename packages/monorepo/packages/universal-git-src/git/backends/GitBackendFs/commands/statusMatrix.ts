...
import { isIgnored as isIgnoredInternal } from "../../../info/isIgnored.ts"
...
export async function statusMatrix({
  ...
  try {
    ...
    if (!repo?.gitBackend) {
      throw new Error('gitBackend is required for statusMatrix')
    }
    ...
    const result = await _walk({
      ...
      map: WalkerMapWithNulls(async function (filepath: string, [head, workdir, stage]: (WalkerEntry | null)[]): Promise<StatusRow | null> {
        ...
        try {
          if (stageType === 'blob' && stage) {
            stageOid = await stage.oid()
          } else {
            stageOid = undefined
          }
        } catch {
          // If we can't get stage oid (e.g., blob object doesn't exist), treat as absent
          // Don't filter out - let the status calculation handle it
          stageOid = undefined
        }
        
        // Get workdir OID if workdir exists and is a blob
        if (workdirType === 'blob' && workdir) {
          try {
          ...
          // Special case: file exists in workdir but not in HEAD or stage
          // We don't actually NEED the sha. Any sha will do
          workdirOid = '42'
        }
        
        // If workdirOid is undefined but stageOid exists and file is in workdir,
        // use stageOid as workdirOid (they should match if file was added)
        // This handles the case where workdir.oid() fails but the file exists
        if (!workdirOid && workdirType === 'blob' && stageOid && stageType === 'blob') {
          // If both workdir and stage are blobs, they should have the same OID if file was added
          // Use stageOid as workdirOid to get correct status [0, 2, 2]
          workdirOid = stageOid
        }
        ...
        // Normalize filepath to remove './' prefix
        const normalizedFilepath = filepath.startsWith('./') ? filepath.slice(2) : filepath
        return [normalizedFilepath, ...result] as StatusRow
      }),
    ...
    // Priority: prefer entries that have both workdir and stage when both exist
    // [0, 2, 2] (workdir + stage) > [0, 0, 3] (only stage) > [0, 2, 0] (only workdir)
    const deduplicated = new Map<string, StatusRow>()
    for (const row of resultArray) {
      const filepath = row[0] as string
      const existing = deduplicated.get(filepath)
      if (!existing) {
        deduplicated.set(filepath, row)
      } else {
        // Compare status values: [head, workdir, stage]
        const existingHead = existing[1] as number
        const existingWorkdir = existing[2] as number
        const existingStage = existing[3] as number
        const newHead = row[1] as number
        const newWorkdir = row[2] as number
        const newStage = row[3] as number
        
        // Prefer entries with both workdir (2) and stage (2) - this is the most complete status
        // [0, 2, 2] means file is in both workdir and stage, and they match
        // [0, 0, 3] means file is only in stage (not in workdir)
        // [0, 2, 0] means file is only in workdir (not in stage)
        const existingHasWorkdir = existingWorkdir === 2
        const existingHasStage = existingStage === 2
        const newHasWorkdir = newWorkdir === 2
        const newHasStage = newStage === 2
        
        // If new entry has both workdir=2 and stage=2, prefer it
        if (newHasWorkdir && newHasStage && (!existingHasWorkdir || !existingHasStage)) {
          deduplicated.set(filepath, row)
        } else if (existingHasWorkdir && existingHasStage && (!newHasWorkdir || !newHasStage)) {
          // Keep existing if it has both and new doesn't
          continue
        } else {
          // Both have or don't have both, prefer entry with higher stage value (more complete)
          // But prioritize stage=2 (matches workdir) over stage=3 (different from workdir)
          if (newStage === 2 && existingStage !== 2) {
            deduplicated.set(filepath, row)
          } else if (existingStage === 2 && newStage !== 2) {
            // Keep existing
            continue
          } else if (newStage > existingStage) {
            deduplicated.set(filepath, row)
          } else if (newStage === existingStage && newWorkdir > existingWorkdir) {
            deduplicated.set(filepath, row)
          }
        }
      }
    ...
    return Array.from(deduplicated.values()) as StatusRow[]
  } catch (err) {
