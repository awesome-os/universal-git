...
export async function writeRef(this: GitBackendFs, ref: string, value: string, skipReflog: boolean = false, cache: Record<string, unknown> = {}): Promise<void> {
  ...
  // Detect if value is a symbolic ref (starts with 'refs/' or 'ref: ')
  // If so, use writeSymbolicRef instead
  const isSymbolicRef = value.startsWith('refs/') || value.startsWith('ref: ')
  if (isSymbolicRef) {
    const symbolicValue = value.startsWith('ref: ') ? value.slice(5).trim() : value
    const objectFormat = await this.getObjectFormat(cache)
...
export async function writeSymbolicRef(this: GitBackendFs, ref: string, value: string, oldOid?: string, newOid?: string, cache: Record<string, unknown> = {}): Promise<void> {
