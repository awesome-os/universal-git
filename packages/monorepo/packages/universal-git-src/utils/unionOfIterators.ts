...
export function* unionOfIterators(sets: Array<Iterator<string>>): Generator<(string | null)[]> {
  ...
  for (let i = 0; i < numsets; i++) {
    ...
    if (heads[i] !== undefined) {
      if (minimum === null || heads[i]! < minimum) {
        minimum = heads[i]!
      }
    }
  }
  ...
  while (true) {
    ...
    minimum = null
    for (let i = 0; i < numsets; i++) {
      if (heads[i] !== undefined && heads[i] === currentMinimum) {
        result[i] = heads[i]!
        heads[i] = sets[i].next().value
      } else {
