...
export async function mergeBlobs({
  ...
  const ourContent = ourContentBuffer ? UniversalBuffer.from(ourContentBuffer).toString('utf8') : ''
  const theirContent = theirContentBuffer ? UniversalBuffer.from(theirContentBuffer).toString('utf8') : ''
  ...
  // Write merged blob using writeBlob with dryRun support
  const oid = await writeBlob({
    fs,
    gitdir,
    blob: UniversalBuffer.from(mergedText, 'utf8'),
    dryRun,
  })
