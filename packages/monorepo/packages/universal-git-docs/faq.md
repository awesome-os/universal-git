...
universal-git just happens to be the most mature successor.

## How does this compare with `nodegit`?

> How is universal-git different from nodegit?
...
> I've noticed that ES6 import of the the module requires import * as git from 'universal-git'.
...
```js
const git = require('universal-git').default
```
...
Default exports are actually really bad for tree-shaking. If you do `import * as git from 'universal-git'` and only use `git.log`, rollup and webpack are smart enough to only bundle `git.log`.
But if you do `import git from 'universal-git'` then they can't do any tree-shaking, because you're importing an Object that could have interdependent functions and side effects.
...
Long answer including a browser solution by @jcubic: [#187](https://github.com/universal-git/universal-git/issues/187)
...
Not yet, but you can go [upvote the issue](https://github.com/universal-git/universal-git/issues/585)
...
universal-git only supports a CORS proxy out of the box. However, all HTTP requests are handled by the http plugin. Therefore, you can swap out the http plugin with a wrapper to inject an HTTP or HTTPS agent that routes requests through the proxy.
This technique only works when using universal-git on Node.js.
...
const { request: delegate } = require('universal-git/http/node')
...
Next, assign this plugin to the http variable instead of universal-git/http/node:
...
With this code in place, universal-git will honor the `http_proxy` and `https_proxy` environment variables.
