...
universal-git comes with a simple CLI tool, named "isogit" because "universal-git" is a lot to type.
It is really just a thin shell that translates command line arguments into the equivalent JS API commands,
so you should be able to run any current *or future* universal-git commands using the CLI.
...
```sh
isogit clone --url=https://github.com/universal-git/universal-git --depth=1 --singleBranch
```
...
git.clone({
  fs: require('fs'),
  dir: process.cwd(),
  url: 'https://github.com/universal-git/universal-git',
  depth: 1,
  singleBranch: true
})
```
