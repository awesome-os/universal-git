...
The `onAuth` callback allows universal-git to request credentials.
...
Since it is a rarely used feature, I'm not including the conversion table directly in universal-git anymore.
But if there's interest in maintaining this table as some kind of function, I'm considering starting an `@universal-git/quirksmode` package to handle these kinds of hosting-provider specific oddities.
