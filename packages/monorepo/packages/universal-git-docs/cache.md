...
Running this code on the `universal-git` repo on my 2018 Macbook Pro takes over 2 minutes!
...
As you can see, you can easily write yourself into a performance trap using `universal-git` commands in isolation.

Unlike canonical `git` commands however, there is a way for `universal-git` commands to cache intermediate results
...
2. universal-git stores data on it by setting Symbol properties.
...
Still nowhere as good as `statusMatrix`, but not everything you might want to do with universal-git can be described by a [`walk`](./walk.html).
