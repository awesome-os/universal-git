...
I'm cloning `universal-git` itself (how meta!).
I'm only cloning a single branch and only to a depth of 10 commits to save time, bandwidth, and browser storage space.
Since GitHub hasn't added CORS headers to the git clone endpoint yet, we have to use a [proxy server](https://cors.universal-git.org/).
...
git.status is a major one. That let's us compare the worktree file to the current branch.
...
Congrats! This just scratches the surface of what you can do with `universal-git`.
