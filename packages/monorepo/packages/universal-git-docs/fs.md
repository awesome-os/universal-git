...
You need to pass a file system into `universal-git` functions that do anything that involves files (which is most things in git).
...
If you're only using universal-git in Node, you can just use the native `fs` module:
...
While ZenFS (see next section) has more features, [LightningFS](https://github.com/universal-git/lightning-fs) might very well fit your needs.
It was designed from scratch for `universal-git` (by the same author) to eek out more performance
...
<script type="importmap">
  {
    "imports": {
      "universal-git": "https://esm.sh/universal-git",
...
An [advanced example usage](https://github.com/universal-git/universal-git/blob/53f2e909030adb1c6ae855b14f3a2474ca93ce71/__tests__/__helpers__/FixtureFS.js#L12) is in the old unit tests for universal-git.
...
There are actually TWO possible interfaces for an `fs` object: the classic "callback" API and the newer "promise" API. If your `fs` object provides an enumerable `promises` property, `universal-git` will use the "promise" API _exclusively_.
...
Internally, `universal-git` wraps the provided "callback" API functions using [`pify`](https://www.npmjs.com/package/pify).
...
<a id="footnote-2">²</a> Right now, universal-git rewrites the file if it needs to change its mode. In the future, if `chmod` is available it will use that.
