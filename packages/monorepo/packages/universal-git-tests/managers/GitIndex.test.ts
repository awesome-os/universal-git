...
test('GitIndex', async (t) => {
  await t.test('GitIndex.from(buffer) - Simple', async () => {
    ...
    const buffer = await fs.read(join(dir, 'simple-index'))
    if (!buffer) {
      throw new Error('Failed to read index file')
    }
    const indexBuffer = typeof buffer === 'string' ? UniversalBuffer.from(buffer) : UniversalBuffer.isBuffer(buffer) ? buffer : UniversalBuffer.from(buffer)
    ...
    // Compare buffers excluding last 20 bytes (timestamp)
    assert.ok(indexBuffer.slice(0, buffer2.length - 20).equals(buffer2.slice(0, -20)))
  })

  await t.test('GitIndex.from(buffer)', async () => {
    const { fs, dir } = await makeFixture('test-GitIndex')
    const buffer = await fs.read(join(dir, 'index'))
    if (!buffer) {
      throw new Error('Failed to read index file')
    }
    const indexBuffer = typeof buffer === 'string' ? UniversalBuffer.from(buffer) : UniversalBuffer.isBuffer(buffer) ? buffer : UniversalBuffer.from(buffer)
    const index = await GitIndex.fromBuffer(indexBuffer)
    const rendering = index.render()
    const expectedRendering = `100644 1db939d41956405f755e69ab570296c7ed3cec99    .babelrc
...
100644 80708a513b7808becff0acfd70dbd3b66a4fb537    test/test-resolveRef.js`
    assert.strictEqual(rendering, expectedRendering)
    const buffer2 = await index.toBuffer()
    // Compare buffers excluding last 20 bytes (timestamp)
    assert.ok(indexBuffer.slice(0, buffer2.length - 20).equals(buffer2.slice(0, -20)))
  })

  await t.test('GitIndex round trip', async () => {
    const { fs, dir } = await makeFixture('test-GitIndex')
    const buffer = await fs.read(join(dir, 'index'))
    if (!buffer) {
      throw new Error('Failed to read index file')
    ...
    assert.ok(buffer2.equals(buffer3))
  })

  await t.test('write unmerged index to disk and read it back', async () => {
    ...
    assert.strictEqual(index.entriesFlat.length, 0)
    ...
    assert.strictEqual(index.entriesFlat.length, 3)
    assert.ok(index.unmergedPaths.includes('a'))

    const entryA = index.entriesMap.get('a')
    assert.ok(entryA, 'Entry a should exist')
    assert.strictEqual(entryA.stages.length, 4)
    assert.strictEqual(entryA.stages[1], index.entriesFlat[0])
    assert.strictEqual(entryA.stages[2], index.entriesFlat[1])
    assert.strictEqual(entryA.stages[3], index.entriesFlat[2])
  })

  await t.test('read existing unmerged index', async () => {
    // Setup
