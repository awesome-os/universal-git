...
`
  ...
  await t.test('param:dir-derives-gitdir', async () => {
    ...
    await init({ fs, dir, defaultBranch: 'main' })
    await writeRef({ fs, gitdir: `${dir}/.git`, ref: 'refs/heads/main', value: 'a'.repeat(40) })
    const result = await expandRef({ fs, dir, ref: 'main' })
    assert.strictEqual(result, 'refs/heads/main')
  })
  ...
  await t.test('error:caller-property', async () => {
    ...
    await init({ fs, dir, defaultBranch: 'main' })
    try {
      await expandRef({ fs, gitdir, ref: 'nonexistent-ref' })
      assert.fail('Should have thrown NotFoundError')
    } catch (error: any) {
  ...
  await t.test('rejects partial SHA (less than 40 characters)', async () => {
    ...
    try {
      await expandRef({ fs, gitdir, ref: 'a'.repeat(39) })
  ...
  await t.test('rejects invalid SHA format (non-hex characters)', async () => {
    ...
    try {
      await expandRef({ fs, gitdir, ref: invalidSha })
      // Should try to expand as ref, not return as SHA
    } catch (error) {
      assert.ok(error instanceof NotFoundError || error instanceof Error)
    }
  ...
  await t.test('handles refs with special characters', async () => {
    ...
    await writeRef({ fs, gitdir, ref: 'refs/heads/feature/branch-name', value: 'a'.repeat(40) })
    const result = await expandRef({ fs, gitdir, ref: 'feature/branch-name' })
  ...
  await t.test('handles HEAD ref expansion', async () => {
    ...
    if (!(await fs.exists(headPath))) {
      await fs.write(headPath, 'ref: refs/heads/main', 'utf8')
    }
    await writeRef({ fs, gitdir, ref: 'refs/heads/main', value: 'a'.repeat(40) })
    // HEAD should be found directly (exact match comes first)
    const result = await expandRef({ fs, gitdir, ref: 'HEAD' })
  ...
  await t.test('handles corrupted packed-refs gracefully', async () => {
    ...
    await fs.write(packedRefsPath, corruptedContent, 'utf8')
  ...
  await t.test('handles empty packed-refs file', async () => {
    ...
    const packedRefsPath = `${gitdir}/packed-refs`
    await fs.write(packedRefsPath, '', 'utf8')
  ...
  await t.test('handles refs/remotes/ with multiple slashes', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    await writeRef({ fs, gitdir, ref: 'refs/remotes/origin/feature/branch', value: 'a'.repeat(40) })
    const result = await expandRef({ fs, gitdir, ref: 'origin/feature/branch' })
  ...
  await t.test('handles refs/remotes/HEAD expansion with multiple slashes', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    await writeRef({ fs, gitdir, ref: 'refs/remotes/origin/feature/HEAD', value: 'a'.repeat(40) })
    const result = await expandRef({ fs, gitdir, ref: 'origin/feature/HEAD' })
...
`
  ...
  await t.test('handles refs/refs/ prefix (double prefix)', async () => {
    ...
    await writeRef({ fs, gitdir, ref: 'refs/refs/heads/main', value: 'a'.repeat(40) })
    const result = await expandRef({ fs, gitdir, ref: 'refs/heads/main' })
...
`
    ...
    await fs.write(packedRefsPath, Buffer.from(packedRefsContent, 'utf8'))
    ...
    const result = await expandRef({ fs, gitdir, ref: 'packed-from-buffer' })
...
`
    ...
    const result = await expandRef({ fs, gitdir, ref: 'only-in-packed' })
  ...
  await t.test('checks refpaths in correct order (refs/tags/ before refs/heads/)', async () => {
    ...
    await writeRef({ fs, gitdir, ref: 'refs/heads/test', value: 'a'.repeat(40) })
    await writeRef({ fs, gitdir, ref: 'refs/tags/test', value: 'b'.repeat(40) })
    
    // Should prefer refs/tags/ over refs/heads/ (comes first in refpaths order)
    const result = await expandRef({ fs, gitdir, ref: 'test' })
  ...
  await t.test('checks refs/ before refs/tags/ and refs/heads/', async () => {
    ...
    await writeRef({ fs, gitdir, ref: 'refs/test', value: 'a'.repeat(40) })
    await writeRef({ fs, gitdir, ref: 'refs/heads/test', value: 'b'.repeat(40) })
    await writeRef({ fs, gitdir, ref: 'refs/tags/test', value: 'c'.repeat(40) })
  ...
  await t.test('checks exact ref match before other paths', async () => {
    ...
    await writeRef({ fs, gitdir, ref: 'test', value: 'a'.repeat(40) })
    await writeRef({ fs, gitdir, ref: 'refs/test', value: 'b'.repeat(40) })
    await writeRef({ fs, gitdir, ref: 'refs/heads/test', value: 'c'.repeat(40) })
...
`
    ...
    const result = await expandRef({ fs, gitdir, ref: 'packed-tag' })
