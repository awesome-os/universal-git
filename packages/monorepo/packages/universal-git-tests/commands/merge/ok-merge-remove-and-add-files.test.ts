...
describe('merge', () => {
  ...
  it('ok:merge-remove-and-add-files', async () => {
        ...
        const mergeOid = await repo.repo.gitBackend.readRef('refs/heads/remove-files') || ''
        ...
        await repo.repo.gitBackend.writeRef('refs/heads/remove-files', beforeMergeOid)
        // Update working directory to match
        await checkout({ repo: repo.repo, ref: 'remove-files', force: true })
