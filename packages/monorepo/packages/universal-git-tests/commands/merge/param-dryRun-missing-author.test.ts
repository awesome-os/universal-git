...
describe('merge', () => {
  ...
  it('param:dryRun-missing-author', async () => {
        ...
        // Delete author from config to ensure no author is found
        const { deleteConfig } = await import('@awesome-os/universal-git-src/index.ts')
        try {
        await deleteConfig({ repo, path: 'user.name' })
        } catch {
        // Config might not exist, that's okay
        }
        try {
        await deleteConfig({ repo, path: 'user.email' })
        } catch {
        // Config might not exist, that's okay
        }
        // Test
