...
describe('merge', () => {
  ...
  it('param:dir-and-gitdir-missing', async () => {
        ...
        assert.notStrictEqual(error, null, 'Should throw error when repo is missing')
        // The error might be about missing repo/fs/dir/gitdir OR about not being able to resolve refs
        // Both are valid error paths
        const errorMessage = (error as any)?.message || ''
        const isParameterError = errorMessage.includes('dir') || errorMessage.includes('gitdir') || errorMessage.includes('repo') || errorMessage.includes('fs')
        const isRefError = errorMessage.includes('Could not find') || errorMessage.includes('not found')
        assert.ok(
        isParameterError || isRefError,
        `Error should mention repo/fs/dir/gitdir requirement or ref resolution. Got: ${errorMessage}`
        )
        })
})
