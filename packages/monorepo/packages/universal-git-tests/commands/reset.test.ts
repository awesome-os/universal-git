...
import { resetToCommit, resolveRef, currentBranch, commit, add, status, listFiles } from '@awesome-os/universal-git-src/index.ts'
...
test('resetToCommit', async (t) => {
  ...
  await t.test('ok:reset-uses-provided-branch-parameter', async () => {
    ...
    const resetOid = await resolveRef({ repo, ref: 'refs/heads/test-branch' })
  ...
  await t.test('ok:reset-uses-current-branch-when-branch-not-provided', async () => {
    ...
    const resetOid = await resolveRef({ repo, ref: `refs/heads/${branchName}` })
  ...
  await t.test('edge:reset-no-reflog-when-oldOid-equals-newOid', async () => {
    ...
    const resetOid = await resolveRef({ repo, ref: 'HEAD' })
  ...
  await t.test('ok:reset-handles-branch-that-does-not-exist-yet', async () => {
    ...
    const branchOid = await resolveRef({ repo, ref: 'refs/heads/new-branch' })
  ...
  await t.test('ok:reset-uses-default-branch-main-when-config-not-exist', async () => {
    ...
    const resetOid = await resolveRef({ repo, ref: 'refs/heads/main' })
  ...
  await t.test('ok:soft-reset-only-updates-HEAD-and-branch-ref', async () => {
    ...
    const indexAfter = await listFiles({ repo })
  ...
  await t.test('ok:soft-reset-keeps-staged-changes', async () => {
    ...
    const stagedStatus = await status({ repo, filepath: 'staged.txt' })
  ...
  await t.test('ok:soft-reset-keeps-working-directory-changes', async () => {
    ...
    const indexFiles = await listFiles({ repo })
  ...
  await t.test('ok:mixed-reset-unstages-all-changes', async () => {
    ...
    const stagedStatus2 = await status({ repo, filepath: 'file2.txt' })
    const stagedStatus3 = await status({ repo, filepath: 'file3.txt' })
  ...
  await t.test('ok:mixed-reset-keeps-working-directory-changes', async () => {
    ...
    const file1Status = await status({ repo, filepath: 'file1.txt' })
    ...
    const newFileStatus = await status({ repo, filepath: 'new-file.txt' })
