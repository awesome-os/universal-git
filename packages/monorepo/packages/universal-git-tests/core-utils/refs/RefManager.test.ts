...
import { RefManager } from '@awesome-os/universal-git-src/core-utils/refs/RefManager.ts'
...
test('RefManager.updateRemoteRefs', async (t) => {
  await t.test('throws InvalidOidError for invalid OID', async () => {
    ...
    await assert.rejects(
      async () => {
        await RefManager.updateRemoteRefs({
          fs,
          gitdir,
          remote: 'origin',
          refs,
  ...
  await t.test('updates remote refs from refs/heads/', async () => {
    ...
    const remoteRef = await RefManager.resolve({ fs, gitdir, ref: 'refs/remotes/origin/master' })
  ...
  await t.test('handles short ref names (without refs/ prefix)', async () => {
    ...
    const remoteRef = await RefManager.resolve({ fs, gitdir, ref: 'refs/remotes/origin/develop' })
    assert.strictEqual(remoteRef, commitOid)
  })
  ...
  await t.test('handles HEAD ref', async () => {
    ...
    const remoteHead = await RefManager.resolve({ fs, gitdir, ref: 'refs/remotes/origin/HEAD' })
  ...
  await t.test('prunes remote refs when prune=true', async () => {
    ...
    const result = await RefManager.updateRemoteRefs({
      fs,
      gitdir,
      remote: 'origin',
      refs,
    ...
    await assert.rejects(
      async () => {
        await RefManager.resolve({ fs, gitdir, ref: 'refs/remotes/origin/old-branch' })
      },
      (error: any) => {
  ...
  await t.test('does not prune when prune=false', async () => {
    ...
    const oldBranch = await RefManager.resolve({ fs, gitdir, ref: 'refs/remotes/origin/old-branch' })
  ...
  await t.test('handles tags when tags=true', async () => {
    ...
    const tagRef = await RefManager.resolve({ fs, gitdir, ref: 'refs/tags/v1.0' })
  ...
  await t.test('skips tags when tags=false', async () => {
    ...
    await assert.rejects(
      async () => {
        await RefManager.resolve({ fs, gitdir, ref: 'refs/tags/v1.0' })
      },
      (error: any) => {
        return error instanceof Error
      }
  ...
  await t.test('writes symbolic refs correctly', async () => {
    ...
    // Verify symbolic ref was created
    const symref = await RefManager.resolve({ fs, gitdir, ref: 'refs/remotes/origin/master' })
