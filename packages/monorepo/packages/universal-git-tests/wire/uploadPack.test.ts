...
import { uploadPack } from '@awesome-os/universal-git-src/git/backends/GitBackendFs/commands/uploadPack.ts'
...
test('uploadPack', async (t) => {
  ...
  await t.test('error handling - sets caller property', async () => {
    ...
    const invalidBackend = {
      getGitdir: () => '/nonexistent/gitdir',
      getFs: () => fs,
      listRefs: async () => { throw new Error('Mock error') },
      readRef: async () => null,
