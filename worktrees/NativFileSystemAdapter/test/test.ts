...
const arr: Array<{ desc: string; fn: (root: FileSystemDirectoryHandle) => Promise<void> }> = []

const t = (desc: string, fn: (root: FileSystemDirectoryHandle) => Promise<void>): void => {
  arr.push({desc, fn})
}

let err: Error | TypeError | DOMException
let handle: any
let existing_handle: any
let subDir: FileSystemDirectoryHandle
let wfs: fs.FileSystemWritableFileStream
let file_name: string
let body: ReadableStream
let dir: FileSystemDirectoryHandle

t('getDirectoryHandle(create=false) rejects for non-existing directories', async root => {
  err = await capture(root.getDirectoryHandle('non-existing-dir')) as any
...
t('getDirectoryHandle() when a file already exists with the same name', async root => {
  ...
  err = await capture(root.getDirectoryHandle('file-name')) as any
  assert(err.name === 'TypeMismatchError')
  err = await capture(root.getDirectoryHandle('file-name', { create: false })) as any
  assert(err.name === 'TypeMismatchError')
  err = await capture(root.getDirectoryHandle('file-name', { create: true })) as any
})

t('getDirectoryHandle() with empty name', async root => {
  err = await capture(root.getDirectoryHandle('', { create: true })) as any
  assert(err instanceof TypeError)
  err = await capture(root.getDirectoryHandle('', { create: false })) as any
  assert(err instanceof TypeError)
})

t('getDirectoryHandle(create=true) with empty name', async root => {
  err = await capture(root.getDirectoryHandle('.')) as any
  assert(err instanceof TypeError)
  err = await capture(root.getDirectoryHandle('.', { create: true })) as any
  assert(err instanceof TypeError)
})
...
t('getDirectoryHandle() with ".." name', async root => {
  ...
  err = await capture(subDir.getDirectoryHandle('..')) as any
  assert(err instanceof TypeError)
  err = await capture(subDir.getDirectoryHandle('..', { create: true })) as any
  assert(err instanceof TypeError)
})
...
t('getDirectoryHandle(create=false) with a path separator when the directory exists', async root => {
  ...
  err = await capture(root.getDirectoryHandle(path_with_separator)) as any
  assert(err instanceof TypeError)
})
...
t('getDirectoryHandle(create=true) with a path separator', async root => {
  ...
  err = await capture(root.getDirectoryHandle(path_with_separator, { create: true })) as any
  assert(err instanceof TypeError)
})

t('getFileHandle(create=false) rejects for non-existing files', async root => {
  err = await capture(root.getFileHandle('non-existing-file')) as any
  assert(err.name === 'NotFoundError')
})
...
t('getFileHandle(create=false) when a directory already exists with the same name', async root => {
  ...
  err = await capture(root.getFileHandle('dir-name')) as any
  assert(err.name === 'TypeMismatchError')
})

t('getFileHandle(create=true) when a directory already exists with the same name', async root => {
  await root.getDirectoryHandle('dir-name', { create: true })
  err = await capture(root.getFileHandle('dir-name', { create: true })) as any
  assert(err.name === 'TypeMismatchError')
})

t('getFileHandle() with empty name', async root => {
  err = await capture(root.getFileHandle('', { create: true })) as any
  assert(err instanceof TypeError)
  err = await capture(root.getFileHandle('', { create: false })) as any
  assert(err instanceof TypeError)
})

t('getFileHandle() with "." name', async root => {
  err = await capture(root.getFileHandle('.')) as any
  assert(err instanceof TypeError)
  err = await capture(root.getFileHandle('.', { create: true })) as any
  assert(err instanceof TypeError)
})
...
t('getFileHandle(create=false) with a path separator when the file exists.', async root => {
  ...
  err = await capture(root.getFileHandle('subdir-name/file_name')) as any
  assert(err instanceof TypeError)
})

t('getFileHandle(create=true) with a path separator', async root => {
  await createDirectory('subdir-name', root)
  err = await capture(root.getFileHandle('subdir-name/file_name', { create: true })) as any
  assert(err instanceof TypeError)
})
...
t('removeEntry() to remove a file', async root => {
  ...
  err = await capture(getFileContents(handle)) as any
  assert(err.name === 'NotFoundError')
})
...
t('removeEntry() on an already removed file should fail', async root => {
  handle = await createFileWithContents('file-to-remove', '12345', root)
  await root.removeEntry('file-to-remove')
  err = await capture(root.removeEntry('file-to-remove')) as any
  assert(err.name === 'NotFoundError')
})
...
t('removeEntry() to remove an empty directory', async root => {
  ...
  arrayEqual(await getSortedDirectoryEntries(root), ['file-to-keep'])
  err = await capture(getSortedDirectoryEntries(handle)) as any
  assert(err.name === 'NotFoundError')
})
...
t('removeEntry() on a non-empty directory should fail', async root => {
  ...
  err = await capture(root.removeEntry('dir-to-remove')) as any
...
t('removeEntry() with empty name should fail', async root => {
  ...
  err = await capture(handle.removeEntry('')) as any
  assert(err instanceof TypeError)
})
...
t('removeEntry() with a path separator should fail.', async root => {
  ...
  err = await capture(root.removeEntry('dir-name/file-name')) as any
  assert(err instanceof TypeError)
})
...
t('abort() aborts write', async root => {
  ...
  err = await capture(promise) as any
  assert(err.name === 'AbortError')
  err = await capture(wfs.close()) as any
  assert(err instanceof TypeError)
  assert(await getFileContents(handle) === '')
  assert(await getFileSize(handle) === 0)
})
...
t('atomic writes: write() after close() fails', async root => {
  ...
  assert(await getFileContents(handle) === 'foo')
  assert(await getFileSize(handle) === 3)
  err = await capture(wfs.write('abc')) as any
  assert(err instanceof TypeError)
})
...
t('atomic writes: truncate() after close() fails', async root => {
  ...
  assert(await getFileContents(handle) === 'foo')
  assert(await getFileSize(handle) === 3)
  err = await capture(wfs.truncate(0)) as any
  assert(err instanceof TypeError)
})
...
t('WriteParams: truncate missing size param', async root => {
  ...
  wfs = await handle.createWritable()
  err = await capture(wfs.write({ type: 'truncate' })) as any
...
t('WriteParams: write missing data param', async root => {
  ...
  wfs = await handle.createWritable()
  err = await capture(wfs.write({ type: 'write' })) as any
  assert(err.name === 'SyntaxError')
})
...
t('WriteParams: seek missing position param', async root => {
  ...
  wfs = await handle.createWritable()
  err = await capture(wfs.write({ type: 'seek' })) as any
  assert(err.name === 'SyntaxError')
})
...
t('createWritable() fails when parent directory is removed', async root => {
  ...
  err = await capture(handle.createWritable()) as any
  assert(err.name === 'NotFoundError')
})
