...
if (
  ...
  globalThis.FileSystemFileHandle.prototype.createWritable = async function (options?: { keepExistingData?: boolean }): Promise<FileSystemWritableFileStream> {
    ...
    const send = (message: any): Promise<any> => new Promise((resolve, reject) => {
      const mc = new MessageChannel()
      mc.port1.onmessage = (evt: MessageEvent) => {
    ...
    const parent = wm.get(this as any)
    if (!parent) throw new Error('Parent not found in WeakMap')
    const path = await root.resolve(parent as any)
  ...
  FileSystemDirectoryHandle.prototype.getFileHandle = async function (name: string, options?: { create?: boolean }): Promise<FileSystemFileHandle> {
    const handle = await orig.call(this, name, options)
    wm.set(handle as any, this as any)
    return handle
  }
}
